...
fn test_initialize_deposit_withdraw_close() {
    ...
    msg!("Initialized transaction succesfull");
    ...
    let vault_state =
        vault::state::VaultState::try_deserialize_unchecked(&mut vault_state_account.data.as_ref())
            .unwrap();
    ...
    msg!("Deposit transaction succesfull");
    ...
    msg!("Withdraw transaction succesfull");
    ...
    msg!("Balance after withdraw: {}", vault_balance_after_deposit);

    // close sol;

    let close_amount: u64 = svm.get_balance(&vault_pda).unwrap();
    ...
    let recent_blockhash = svm.latest_blockhash();
    let transaction3 = Transaction::new(&[&payer], message, recent_blockhash);
    let tx4 = svm.send_transaction(transaction3).unwrap();

    msg!("Close transaction succesfull");
    ...
    msg!("Balance after deposit: {}", user_balance_after_close);
}
